...
- `[x]` 2026-09-09 — Kişisel veri ve yerel yol arındırması tamamlandı: Cargo `.cargo/config.toml` dosyalarına `--remap-path-prefix` eklenerek `anticore-cli.exe`, `Anticore.exe` ve paketlerden tüm yerel geliştirici Windows yolları (`C:\Users\...`) tamamen temizlendi. Git commit geçmişindeki yazar adresi kurumsal `Monolith Works <info@monolithworks.com.tr>` olarak rebase edildi (0 kişisel e-posta). Paketler yeniden üretildi.
