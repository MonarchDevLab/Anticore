...
$root = 'c:\Users\Ouroboros\Desktop\Personel\Branding\Uygulama\Anticore\antikor'
...
$nsis = "$root\desktop\src-tauri\target\release\bundle\nsis\Anticore_0.3.0_x64-setup.exe"
if (Test-Path $nsis) {
    Copy-Item $nsis "$root\dist\Anticore_0.3.0_x64-setup.exe" -Force
}
$msi = "$root\desktop\src-tauri\target\release\bundle\msi\Anticore_0.3.0_x64_en-US.msi"
if (Test-Path $msi) {
    Copy-Item $msi "$root\dist\Anticore_0.3.0_x64_en-US.msi" -Force
}

# 6. Portable ZIP Paketi Olustur
$zipTarget = "$root\dist-portable\Anticore_0.3.0_x64-portable.zip"
...
Copy-Item $zipTarget "$root\dist\Anticore_0.3.0_x64-portable.zip" -Force
